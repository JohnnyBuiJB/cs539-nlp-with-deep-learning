...
class SingleQueryScaledDotProductAttention(nn.Module):    
    ...
    def __init__(self, enc_hid_dim, dec_hid_dim, kq_dim=512):
        super().__init__()
        self.wq = nn.Linear(in_features=dec_hid_dim, out_features=kq_dim).to(dev)
        self.wk = nn.Linear(in_features=2 * enc_hid_dim, out_features=kq_dim).to(dev)
        self.wv = nn.Linear(in_features=2 * enc_hid_dim, out_features=2 * enc_hid_dim).to(dev)
        self.kq_dim = kq_dim
    ...
    # encoder_outputs [s,b,eh]
    # hidden [b,kq]
    def forward(self, hidden, encoder_outputs):
        encoder_outputs = encoder_outputs.permute(1,0,2) # [b,s,eh]
        
        b = encoder_outputs.shape[0]    # source length
        s = encoder_outputs.shape[1]    # batch size
        eh = encoder_outputs.shape[2]    # encoder hidden layer size
        kq = self.kq_dim
        
        q = self.wq(hidden).unsqueeze(1) # [b,1,kq]
        k = self.wk(encoder_outputs).permute(0,2,1)     # [b,s,kq] -> [b,kq,s]
        v = self.wv(encoder_outputs)     # [b,s,eh]
        
        score = torch.bmm(q, k)     # [b,1,s]
        score = torch.div(torch.bmm(q, k), np.sqrt(kq)) # [b,1,s]
        
        alpha = torch.softmax(score, len(score.shape) - 1) # [b,1,s]
            
        attended_val = torch.bmm(alpha, v).squeeze(1) # [b,1,eh] -> [b,eh]
        alpha = alpha.squeeze(1) # [b,s]
        
        assert attended_val.shape == (b, eh)    # [b,eh]
        assert alpha.shape == (b, s)   # [b,s]
